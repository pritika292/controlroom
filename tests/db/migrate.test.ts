...
describeIfDb("migrate", () => {
  ...
  beforeEach(async () => {
    ...
    await client.query("DROP TABLE IF EXISTS ai_usage");
  ...
  it("runs cleanly on an empty DB and records 6 entries in _migrations", async () => {
    ...
    expect(result.applied).toContain("006_ai_usage.sql");
  ...
  it("is idempotent: re-running leaves 6 entries, nothing re-applied", async () => {
